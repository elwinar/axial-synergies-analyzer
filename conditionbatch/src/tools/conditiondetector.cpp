...
QString ConditionDetector::detect(Record * record)
{
	...
	if(!record->containsMarker("BALAVD") && !record->containsMarker("BALAVG") && !record->containsMarker("BALARD") &&	!record->containsMarker("BALARG"))
	{
		condition = "STD";
	}
	else
	{
		...
		for(unsigned int duration = 1; duration <= record->durationMarkers(); duration ++)
		{
