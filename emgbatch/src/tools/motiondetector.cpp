...
bool MotionDetector::run(QPair<QString, QString> fixed, QPair<QString, QString> mobile, int plan)
{
     ...
     */
    for(unsigned int time = 1; time < _record->durationMarkers(); time++)
    {
    ...
    _speeds.insert(1, 0);
    _peak = 1;
    for(unsigned int time = 2; time < _record->durationMarkers(); time++)
    {
     ...
     */
    _end = 0;
    for(unsigned int time = _peak; time <= _record->durationMarkers(); time++)
    {
        if(_speeds.contains(time) && _speeds.value(time) < threshold())
        {
            _end = time;
